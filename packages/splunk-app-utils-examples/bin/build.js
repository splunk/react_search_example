...
const runCommands = {
    win32: {
        ...
        link: () => shell.exec('mklink /D "%SPLUNK_HOME%\\etc\\apps\\splunk-app-utils-examples" "%cd%\\stage"'),
    },
    nix: {
        build: () => shell.exec('export NODE_ENV=production && ./node_modules/.bin/webpack -p'),
        link: () => shell.exec('ln -s $PWD/stage $SPLUNK_HOME/etc/apps/splunk-app-utils-examples'),
    },
};
